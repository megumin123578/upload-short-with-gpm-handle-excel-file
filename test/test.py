...
class App(tk.Tk):
    ...
    def _build_inputs(self):
        ...
        ttk.Button(btns, text="AI Generate", command=self.ai_generate_titles_descs).pack(side=tk.LEFT, padx=6)
    ...
    def show_loading(self):
        ...
        ttk.Label(self.loading_win, text="Generating... ⏳").pack(expand=True, pady=20)
    ...
    def ai_generate_titles_descs(self):
        prompt = (
            "Generate exactly 5 YouTube short titles (each under 100 characters). "
            "Return only the titles, one per line, no introduction text.\n"
            "Topic: ASMR slime relaxing, oddly satisfying."
        )
        ...
        def worker():
            ...
            try:
                text = generate_text(prompt, model="llama3.1")

                # tách dòng, bỏ intro hoặc dòng rác
                lines = [l.strip("-• ").strip() for l in text.split("\n") if l.strip()]

                # lọc bỏ dòng mở đầu không phải tiêu đề
                titles = [l for l in lines if not l.lower().startswith("here are")]
                ...
                if titles:
                    self.txt_titles.delete("1.0", tk.END)
                    self.txt_titles.insert("1.0", "\n".join(titles))

                self._set_status("AI done ✅")
            except Exception as e:
